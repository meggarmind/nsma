...
export class ConfigWatcher {
  ...
  /**
   * Refresh stats from disk for all active projects
   * Called on each daemon sync cycle to ensure stats are always current
   */
  async refreshAllStats() {
    ...
    for (const project of activeProjects) {
      if (project.promptsPath) {
        const result = await this.refreshStats(project);
        if (result.success) {
          refreshed++;
        }
      }
    ...
    if (refreshed > 0) {
      console.log(`  Stats refresh: ${refreshed} projects updated from disk`);
    }
