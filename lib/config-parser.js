...
export class ConfigParser {
  ...
  async autoImport(existingProject = null) {
    ...
    // Use first found config file (priority order in supportedFiles)
    const primaryConfig = configFiles[0];
    const parsedConfig = await this.parseConfigFile(primaryConfig.filepath);

    // If existing project data, merge intelligently
    if (existingProject) {
      return {
        ...this.mergeWithExisting(parsedConfig, existingProject),
        configSource: primaryConfig.filename,
