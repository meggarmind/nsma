import { readFile, writeFile, mkdir, readdir, unlink, rename, chmod } from 'fs/promises';
...
export async function getSettings() {
  ...
  // Supports both NOTION_* (dashboard) and NSMA_NOTION_* (plugin) conventions
  if (process.env.NOTION_TOKEN || process.env.NSMA_NOTION_TOKEN) {
    settings.notionToken = process.env.NOTION_TOKEN || process.env.NSMA_NOTION_TOKEN;
  }
  if (process.env.NOTION_DATABASE_ID || process.env.NSMA_NOTION_INBOX_DB_ID) {
    settings.notionDatabaseId = process.env.NOTION_DATABASE_ID || process.env.NSMA_NOTION_INBOX_DB_ID;
  }
...
export async function saveSettings(settings) {
  ...
  // This protects sensitive data like API keys stored in the settings file
  try {
    await chmod(SETTINGS_FILE, 0o600);
  } catch (error) {
    // Non-fatal: permissions might fail on some filesystems (e.g., Windows)
    console.warn('[storage] Could not set file permissions:', error.message);
  }
 ...
 * @param {string} filename - The filename of the inbox item to delete
 * @returns {Promise<{deleted: boolean, filename: string}>}
 */
export async function deleteInboxItem(filename) {
  const sourcePath = join(INBOX_PATH, 'pending', filename);
 ...
 * @param {string} filename - The filename of the inbox item to archive
 * @returns {Promise<{archived: boolean, filename: string, from: string, to: string}>}
 */
export async function archiveInboxItem(filename) {
  const sourcePath = join(INBOX_PATH, 'pending', filename);
  const archiveDir = join(INBOX_PATH, 'archived');
