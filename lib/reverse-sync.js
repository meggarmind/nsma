...
export class ReverseSyncProcessor {
  ...
  async syncProject(project) {
    ...
    const results = {
      ...
      skipped: 0,
      errors: [],
      updatedItems: []  // Track successfully updated files for UI display
    };
    ...
    for (const [folder, files] of Object.entries(filesByFolder)) {
      ...
      for (const file of files) {
        try {
          ...
          // Note: error messages may be truncated, so check for partial matches
          const errorType = error.message?.includes('shared with your')
            ? 'permission'
            : error.message?.includes('404') ? 'deleted' : 'other';
  ...
  async handleError(file, error, project) {
    ...
    // Note: error messages may be truncated, so check for partial matches
    const isPermissionError = errorMessage.includes('shared with your');
    ...
    // Permission error - page exists but integration can't access it
    if (isPermissionError) {
      await this.applyErrorMode(file, project,
        'Page not shared with integration (check Notion sharing settings)');
      return;
    }

    // For 404 (page actually deleted in Notion), apply errorMode
    if (is404) {
      await this.applyErrorMode(file, project, 'Notion page deleted or not found');
      return;
    }
