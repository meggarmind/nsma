...
export class SyncProcessor {
  ...
  async processProjectItems(project, items, settings, notion, isInbox = false) {
    ...
    for (const { page, item } of items) {
      ...
      try {
        ...
        } else if (settings.anthropicApiKey && project.aiPromptEnabled !== false) {
          // AI expansion is enabled (default true) and API key is configured
  ...
  async run() {
    ...
    if (!this.skipReverseSync) {
      ...
      // Process active projects that have reverse sync enabled
      const reverseSyncProjects = projects.filter(p =>
        p.active && p.reverseSyncEnabled !== false
      );

      for (const project of reverseSyncProjects) {
        // Use project-specific error mode or default to 'skip'
