...
// Dynamic service name based on NSMA_INSTANCE environment variable
// Supports both legacy (notion-sync.service) and new naming (nsma-daemon-{instance}.service)
const INSTANCE = process.env.NSMA_INSTANCE;
const SERVICE_NAME = INSTANCE
  ? `nsma-daemon-${INSTANCE}.service`
  : 'notion-sync.service'; // Fallback for backward compatibility
