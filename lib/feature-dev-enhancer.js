...
export class FeatureDevEnhancer {
  ...
  async enhance(item, project, baseContent) {
    ...
    return this.formatEnhancedSections(response.content[0].text, item, project);
  }
...
Provide detailed feature-dev analysis with Architecture Analysis, Code Exploration Results, Implementation Blueprint, and Testing Strategy sections.`;
   ...
   * @returns {string}
   */
  formatEnhancedSections(aiResponse, item, project) {
    let output = `
...
`;
    ...
    // Add skill instruction section if enabled (hybrid mode)
    if (this.settings.featureDevSkillEnabled !== false) {
      const relatedFiles = project.modules?.find(m => m.name === item.affectedModule)?.filePaths || [];
      const filesStr = relatedFiles.length
        ? relatedFiles.map(f => `\`${f}\``).join(', ')
        : 'See Related Files section above';

      output += `

---

## Deep Exploration (Claude Code Session)

Before implementing, run the \`/feature-dev:feature-dev\` skill for real-time codebase analysis.

**Why:** The analysis above was generated during sync without codebase access. The skill will:
- Explore actual file contents and structure
- Use LSP for precise symbol navigation
- Validate the Implementation Blueprint
- Discover patterns not visible during sync

**Focus files:** ${filesStr}
`;
    }
